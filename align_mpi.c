 ...
 */
  ...
  */
	...
	int local_count = 0; // numero di pattern trovati localmente
	for (int i = 0; i < local_pat_number; i++) {
	...
	if (rank == 0) {
		recv_counts = (int *)malloc(size * sizeof(int)); // numero di pattern trovati per ogni processo
		displs = (int *)malloc(size * sizeof(int)); // offset per ogni processo (da dove)
	}
