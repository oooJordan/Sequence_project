 ...
 */
 void increment_matches(int pat, unsigned long *local_pat_found, unsigned long *pat_length, int *seq_matches) {
     //perchè il numero di iterazioni pat_length[pat] è noto in anticipo
	 //ogni iterazione ha un carico computazionale uniforme
 ...
 */
	...
	/*  ricerca parallela dei pattern nella sequenza di DNA utilizzando OpenMP
	il numero totale di corrispondenze (local_matches) viene accumulato in modo 
	sicuro con una operazione di riduzione
	se (lind,start) non fossero private, più thread potrebbero modificarle simultaneamente, 
	 portando a risultati errati perché i valori si sovrascriverebbero. */
	#pragma omp parallel for schedule(dynamic,64) reduction(+:local_matches) private(lind, start)
	...
	// 2. Riduzione del conteggio delle occorrenze nella sequenza
