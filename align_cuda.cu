    ...
    unsigned long long* d_pat_found, int* d_pat_matches)
{
    ...
    if (match) {
		if (atomicCAS(&d_pat_found[patIndex], ULLONG_MAX, seqStart + offset) == ULLONG_MAX) {
			...
			//per controllare che il valore sia il più piccolo di tutti 
			//dato che il problema richiede la prima occorrenza da sinistra
 ...
 */
	...
	/* 4.2 Determinazione dimensione chunk e altri valori utili per la sequenza*/
	...
	unsigned long dimBlockX = 48;
	unsigned long dimBlockY = 8;
    dim3 block(dimBlockX, dimBlockY);

	//sequenza suddivisa in chunk sovrapposti, per evitare che un pattern venga spezzato ai confini dei blocchi.
	for (unsigned long seqStart = 0; seqStart < seq_length; seqStart += (chunkSize - overlap)) {
		...
		// Iterazione sui pattern in batch (Poiché il numero totale di pattern può essere molto grande)
		for (int patStart = 0; patStart < pat_number; patStart += batchSize) {
            ...
            // Dimensioni griglia (gridX = posizioni sequenza tra blocchi di thread, gridY = pattern tra blocchi di thread)
